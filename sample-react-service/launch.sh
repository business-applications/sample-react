...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "sample-react-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name sample-react-service apps/sample-react-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app sample-react-service:1.0-SNAPSHOT
    oc expose svc/sample-react-service
else

	echo "Launching the application locally..."
	pattern="sample-react-service"
	files=( $pattern )
	cd ${files[0]}
